...
export default function ActionBar({
  ...
  const examples = [{
    title: "group theory", content: `concept $group(G);
concept x in S;
concept x eq y;
...
know if G, g: G is group, g in G {
  ...
  \\*{G, g, \\inv{G, g} } eq \\1{G}
    ...
    title: "set theory", content: `/* The following examples are LiTeX versions of Chapter 3 from Terence Tao's 
...
`
    ...
    title: "syllogism", content: `/* The file contains an example of syllogism(三段论). It turns out that It just 
...
//   \\*{G, g, \\inv{G, g} } eq \\1{G}
// };` }, {
//     title: "intergers and rationals", content: `concept $natural(x);
...
// };`
//   }, {
//     title: "peano axioms", content: `/* Define the concept of natural number (nat). */
...
//   }, {
//     title: "README example", content: `/* LaTeX version
...
// } god is mortal;`
//   }, {
//     title: "the foundation of geometry", content: `concept A is point;
// operator \\line{A,B}: all A,B are point;
// concept commutative $=(A,B);
// know if A,B,C: all A,B,C are point, \\line{A,B} = \\line{A,C} {
//   \\line{B,C} = \\line{A,B};
// }`
//   }]
  ...
  return (
    <Box display={"flex"}>
      ...
      <Box flex={1}>
        {examples.map((example, index) => <Typography key={index} variant="caption" lineHeight={"32px"} mx={1} color="primary" sx={{ cursor: "pointer", textDecorationLine: "underline" }} onClick={() => { changeExample(example.content) }}>{example.title}</Typography>)}
      </Box>
